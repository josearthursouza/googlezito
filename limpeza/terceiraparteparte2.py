...
for indice_conteudos in range(70,137):
...
current_escrever = open("palavras_ids_B (1)", "a")    
palavras_escritas = 0
...
for palavra_escrever in dicionario:
    ...
    if palavras_escritas == int(1000*1.1222**documentos_escritos):
        ...
        documentos_escritos += 1
        nome_escrever = "palavras_ids_B (" + str(documentos_escritos)+ ")"
...
arquivo = open('palavras_ids_A (1)', "r")
acorrigir = arquivo.read()
mauelemento = acorrigir.find('26')
corrigido = acorrigir[:mauelemento] + acorrigir[mauelemento + 3:]
arquivo.close()
arquivo = open('palavras_ids_A (1)', 'w')
arquivo.write(corrigido)
arquivo.close()